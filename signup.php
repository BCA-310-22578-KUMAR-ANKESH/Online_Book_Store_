...
if (!preg_match("/^[A-Za-z ]+$/", $name)) {
    echo "<script>alert('Full Name should contain only alphabets');</script>";
    echo "<script>window.location='signup.php';</script>";
    exit();
}

// Contact Number: only digits with optional country code (+)
if (!preg_match("/^[6-9][0-9]{9}$/", $contactno)) {
    echo "<script>alert('Enter a valid Indian mobile number');</script>";
    exit();
}
...
<body class="animsition">
	...
	<section class="bg0 p-t-104 p-b-116">
       ...
       class="form-control"
       pattern="(\+91)?[0-9]{10}"
       title="Enter 10-digit number with optional +91 country code"
       required>
						</div>
						<div class="bor8 m-b-20 how-pos4-parent">
